...
같은 기능 요청("반복 일정 기능 추가")을 (1) 단일 에이전트 1-shot 생성과 (2) 이 하네스의 리뷰 파이프라인, 두 조건으로 각각 실행하고 동일 루브릭으로 채점했습니다.

| | PRD (12점) | Plan (10점) |
|---|---|---|
| 1-shot (리뷰 없음) | 7.0 | 4.5 |
| 하네스 (리뷰 반영 최종본) | 10.5 | 9.5 |

가장 눈에 띄는 사례: 두 조건 모두 초안 단계에서는 "멱등성(중복 생성 방지)"과 "배치 시작 시점 스냅샷 기준 처리"라는 서로 다른 두 요구사항을 같은 메커니즘으로 해결했다고 착각하는 동일한 설계 결함을 갖고 있었습니다. `plan-architect` 서브에이전트(실제 호출, 사전 각본 없음)가 이 구조적 결함을 지적했고, 반영 후 `plan-critic`이 검증해 APPROVE했습니다 — 리뷰 프로세스가 없었다면 두 조건 모두에서 잡히지 않았을 결함입니다.

모든 항목에서 하네스가 이긴 것은 아닙니다 — 예를 들어 배포 롤백 전략은 1-shot 결과물이 더 상세했습니다(하네스의 Plan 템플릿에 전용 섹션이 없었기 때문). 채점 근거, 실제 리뷰 전문, 비용 트레이드오프까지 전부 [examples/recurring-task-case-study](examples/recurring-task-case-study)에 투명하게 공개되어 있습니다.
