...
### Agent A: 요구사항 분석 (Explore agent, model=haiku)
...
### Agent B: 환경 및 제약 분석 (Explore agent, model=haiku)
...
### Agent C: 품질 레퍼런스 분석 (Explore agent, model=haiku)
...
> **모델 권장**: 이 단계(설계 대안 비교, 아키텍처 결정)는 판단력이 핵심이므로, 이 커맨드를 실행하는 세션 자체를 **opus**로 두는 것을 권장한다. Phase 3~4의 리뷰 서브에이전트는 sonnet으로 충분하다 — 리뷰는 이미 작성된 설계를 검증하는 작업이라 opus 수준의 창의적 판단보다 꼼꼼함이 더 중요하다.
...
Plan 초안 작성 완료 후, **plan-architect agent** (subagent_type="plan-architect", model=sonnet)를 실행한다.
...
Architect 리뷰 완료 후, **plan-critic agent** (subagent_type="plan-critic", model=sonnet)를 실행한다.
